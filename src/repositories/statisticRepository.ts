...
class StatisticRepository extends BaseRepository<Statistic> {
    ...
    public async sum(field: keyof IGameStat, countOptions?: Record<string, any>): Promise<number> {
        return await this.repository.sum(field, countOptions);
    }

    public async max(field: keyof IGameStat, maxOptions?: Record<string, any>): Promise<number> {
        return await this.repository.max(field, maxOptions);
    }

    public async min(field: keyof IGameStat, minOptions?: Record<string, any>): Promise<number> {
