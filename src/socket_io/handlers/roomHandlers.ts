...
const roomHandlers = ({ io, socket, gameService }: {
    ...
    function join(roomId: string): void {
        ...
        if (!gameService.joinRoom(roomId, user?.login)) {
            socket.emit("room:join", false, "The room is full or the game has already started!");
            return;
        }

        socket.join(roomId);
        socket.emit("room:join", true, `Success, ${gameService.user?.login ?? "user"} join!`);
    }
    ...
    function leave(): void {
        ...
        socket.emit("room:leave", true, `Success, ${gameService.user?.login ?? "user"} left!`);
        socket.leave(roomId);
    }
